...
# 1*1 3*3 1*1
# 第一个1*1用来缩减维度，这样3*3的计算量就会下降，最后一个1*1用来恢复维度
...
class P3D(nn.Module):
    ...
    # block: Bottleneck
    # layers: 长度是block的数量，其中的每个值对应该block包含多少个重复的残差结构
    # shortcut_type: 残差结构采取的形式，默认是'B'，指明残差的具体类型
    # self.avgpool和ResNet网络中最后的7*7的均值池化一样，只不过这里采用的是5*5，主要是因为输入帧的大小是160*160，和ResNet中224*224大小的图像不同
    def __init__(self, block, layers, modality='RGB',
        ...
        # 如果输入是视频帧（也就是图像），channel就是3，输入是光流，就是2
    ...
    # 定义了block个数（也就是4）的层内容
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1):
        num_layers = blocks
        num_input_features = planes
        downsample = None
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    # 该输入数据包含10个clip，每个clip包含16帧图像，每帧图像是160*160的3通道图像
